...
import AccountDialog from 'src/components/auth/AccountDialog';
import { RegisterDialog } from 'src/components/auth/RegisterDialog';
import { LoginFormDialog } from 'src/components/auth/LoginFormDialog';
import { useLoginModal } from 'src/components/auth/LoginFormDialog/action';
import { useAccountModal } from 'src/components/auth/AccountDialog/action';
...
import type { NavMainProps, NavItemDataProps } from './nav/types';
...
export function MainLayout({
  ...
  const navData: NavItemDataProps[] = useMemo(() => {
    if (categories) {
      return categories.filter(item => item.parent_category_id == null).map((item) => ({
        path: `/category/${item.handle}`,
        title: item.name,
        children: item.category_children.length ? [
          {
            items: item.category_children.map((child) => ({
              path: `/category/${child.handle}`,
              title: child.name,
            } as NavItemDataProps))
          }
        ] : undefined
  ...
  const _account: AccountDrawerProps['data'] = [
    ...
    // { label: 'Đổi mật khẩu', href: '#', icon: <Iconify icon="solar:shield-keyhole-bold-duotone" /> },
  ];
  const renderHeader = () => {
      ...
      rightArea: (
        <>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: { xs: 1, sm: 1.5 } }}>
            ...
            {!authenticated ? <> {/** @slot Sign in button */}
              <SignInButton />
